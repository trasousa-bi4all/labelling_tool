...
def create_dash_app(username: str, requests_pathname_prefix: str = None) -> dash.Dash:
    ...
    annotation_types = ["Com Implante", "Sem Implante"]
    ...
    button_group = html.Div(
        dbc.RadioItems(
            ...
            options=[
                {"label": "Bad", "value": 1, "label_id": "bad_option"},
                {"label": "Inconclusive", "value": 2, "label_id": "neutral_option"},
                {"label": "Good", "value": 3, "label_id": "good_option"},
            ],
            value=None,
        ),
    ...
    with open("assets/annotation/Howto.md", "r") as f:
    ...
    blob_container = blob_storage()
    db = sql_db()

    filelist_id, filelist = db.get_n_orthos(10)

    if not len(filelist):
        raise Exception("There are no files to label.")

    server = app.server

    fig = px.imshow(
        skio.imread(blob_container.get_image_from_container("raw", filelist[0])),
        binary_backend="jpg",
    ...
    image_annotation_card = dbc.Card(
        id="imagebox",
        children=[
            dbc.CardHeader(html.H2("Imagem a classificar")),
            dbc.CardBody(
                [
                    dcc.Graph(
                        id="graph",
            ...
            dbc.CardBody(
                [
                    ...
                    dbc.Row(
                        dbc.Col(
                            [
                                html.H3("Tipo de Ortopantomografia"),
                                dcc.Dropdown(
                    ...
                    dbc.Row(
                        dbc.Col([html.H3("Qualidade do Tratamento"), button_group])
                    ),
                ]
            ),
            dbc.CardFooter(
    ...
    navbar = dbc.Navbar(
        dbc.Container(
            [
                            ...
                            html.A(
                                html.Img(
                                    src=app.get_asset_url(
                                        "Logo_SmileUp_ClinicasDentarias.png"
                                    ),
                                    height="30px",
                                ),
                                href="https://smileup.pt",
                            )
                        ),
                ...
                dbc.Row(
                    [
                        dbc.Col(dbc.NavbarBrand(username)),
                        dbc.Col(
                            [
    ...
    def send_figure_to_graph(
        ...
        if annotations_table_data is not None:
            ...
            fig = px.imshow(
                skio.imread(blob_container.get_image_from_container("raw", filename)),
                binary_backend="jpg",
                color_continuous_scale="gray",
            )
            fig.update_layout(
    ...
    # [State("image_files", "data"), State("annotations-store", "data")]
    # )
    def submit_click(n_clicks, image_files_data):
        image_id = image_files_data["files"][image_files_data["current"]]
        db = sql_db()
        db.update_ortho_count(image_id)
